...
- [サイドバーにReact Webviewを表示する（サンプル）.md](spec/サイドバーにReact%20Webviewを表示する.md) ── issue #6対応。Activity Barの左サイドバーに表示するReact Webviewサンプルの仕様。ボタン押下で既存のsendFileToApiコマンドを呼び出す
...
- [0004-サイドバーReact-Webviewサンプルの技術構成を決める.md](ddr/0004-サイドバーReact-Webviewサンプルの技術構成を決める.md) ── issue #6対応。Webviewバンドラー（esbuild）選定・ディレクトリ構成・拡張ホストとのメッセージパッシング方式の決定
